...
typedef struct 
{
 ...
 int f;                 //move up, down, look up, down
}keys; keys K;
...
void movePlayer()
{
 ...
 if(K.a ==1 && K.f==0){ P.a-=4; if(P.a<  0){ P.a+=360;}}  
 if(K.d ==1 && K.f==0){ P.a+=4; if(P.a>359){ P.a-=360;}}
 int dx=M.sin[P.a]*10.0; 
 int dy=M.cos[P.a]*10.0; 
 if(K.w ==1 && K.f==0){ P.x+=dx; P.y+=dy;}
 if(K.s ==1 && K.f==0){ P.x-=dx; P.y-=dy;}
 //strafe left, right
 if(K.sr==1){ P.x+=dy; P.y-=dx;}
 if(K.sl==1){ P.x-=dy; P.y+=dx;}
 //move up, down, look up, look down
 if(K.a==1 && K.f==1){ P.l-=1;}
 if(K.d==1 && K.f==1){ P.l+=1;}
 if(K.w==1 && K.f==1){ P.z-=4;}
 if(K.s==1 && K.f==1){ P.z+=4;}
}
...
float ht=0, ht_step=(float)Textures[wt].w/(float)(x2-x1);
 ...
 for(x=x1;x<x2;x++)
 {
  ...
  float vt=0, vt_step=(float)Textures[wt].h/(float)(x2-x1);
  //Clip Y
  if(y1< 0){ht-=vt_step*y1; y1= 0;} //clip y 
  if(y2< 0){ y2= 0;} //clip y 
